...
indir        <- "/storage/research/giub_geco/data_2/scratch/phelpap/ModESim/m020_tidy/Total_PET_1420"
outfile_pcwd <- "/storage/research/giub_geco/data_2/scratch/phelpap/ModESim/m020_tidy/Collected_Total_PET_1420/Total_PET" # adjust path to where the file should be written to



# 1) Define filenames of files to collect:  -------------------------------
filnams_pcwd <- list.files(indir, pattern = "ModESim_pcwd_(LON_[0-9.+-]*)_tot_pet.rds", full.names = TRUE)
...
prepare_write_nc2 <- function(df_cwd, varname){
  ...
  df_cwd <- df_cwd |>
    dplyr::select(lon, lat, year, tot_pet) |>
    arrange(year, lat, lon)

  arr <- array(
    unlist(df_cwd$tot_pet),
    dim = c(
...
obj_pcwd <- prepare_write_nc2(df_pcwd, varname="total_pet")
...
rgeco::write_nc2(
  ...
  att_title      = "Annual Total Potential Evapotranspiration (PET)",
  att_history    = sprintf(
