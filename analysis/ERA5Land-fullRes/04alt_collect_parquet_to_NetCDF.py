...
def main():
    ...
    def dask_2_xarray(ddf, indexname='index'):
        ...
        for key in ddf.columns:
            print(f"{dtdt.now().strftime('%Y-%m-%d %H:%M:%S')}: dask_2_xarray of column {key}", flush = True)
            ds[key] = (indexname, ddf[key].to_dask_array().compute_chunk_sizes())
        return ds
                
    # use:
    print(f"{dtdt.now().strftime('%Y-%m-%d %H:%M:%S')}: Start preparing for output: dask_2_xarray", flush=True)
    ds = dask_2_xarray(ddf2)

    # postprocess
    print(f"{dtdt.now().strftime('%Y-%m-%d %H:%M:%S')}: Start preparing for output: drop_vars", flush=True)
    ds = ds.drop_vars(['LON_str','year'])
    print(f"{dtdt.now().strftime('%Y-%m-%d %H:%M:%S')}: Start preparing for output: set_coords", flush=True)
    ds = ds.set_coords(['lat', 'lon','date'])

    # fix date type
    print(f"{dtdt.now().strftime('%Y-%m-%d %H:%M:%S')}: Start preparing for output: to_datetime", flush=True)
    ds.date.values = pd.to_datetime(ds.date.values)
    # Replace (Row)-Index by Lon,Lat,Date combination
    print(f"{dtdt.now().strftime('%Y-%m-%d %H:%M:%S')}: Start preparing for output: drop_indexes", flush=True)
    ds = ds.drop_indexes(['index'])
    print(f"{dtdt.now().strftime('%Y-%m-%d %H:%M:%S')}: Start preparing for output: set_xindex", flush=True)
    ds = ds.set_xindex(['lon', 'lat','date'])
    print(f"{dtdt.now().strftime('%Y-%m-%d %H:%M:%S')}: Start preparing for output: unstack", flush=True)
    ds = ds.unstack(["index"])
