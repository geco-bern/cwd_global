...
```{r}
regional_results_1420 <- readRDS("~/cwd_global/data/regionalResults_1420_1.RData") ## Model set 1
regional_results_1850 <- readRDS("~/cwd_global/data/regionalResults_1850_1.RData") ## Model set 1
...
# 
#saveRDS(ensemble_trends, file="~/cwd_global/data/1420_100yr_trends.RData")  ## 100 year trends
saveRDS(ensemble_trends, file="~/cwd_global/data/1420_30yr_trends.RData")  ## 30 year trends

#################################### 1850 epoch ############################################
#running window trend analysis with 30 or 100 year windows - 1850 epoch
...
#      xlab = "Year", ylab = "Trend (mm/yr)")
# 
#saveRDS(ensemble_trends_1850, file="~/cwd_global/data/1850_100yr_trends.RData") 
saveRDS(ensemble_trends_1850, file="~/cwd_global/data/1850_30yr_trends.RData")

```
Calculate signal (ensemble mean) and noise (standard deviation) for each region: 
- read in file depending on which trends to visualise; only have to change title description for plots

```{r}
###################### 1420 epoch ##########################
#ensemble_trends <- readRDS("~/cwd_global/data/1420_30yr_trends.RData") ### 30 year trends
ensemble_trends <- readRDS("~/cwd_global/data/1420_30yr_trends.RData") ## 100 year trends
...
###################### 1850 epoch ##########################
#ensemble_trends_1850 <- readRDS("~/cwd_global/data/1850_30yr_trends.RData") ### 30 year trends
ensemble_trends_1850 <- readRDS("~/cwd_global/data/1850_30yr_trends.RData") ## 100 year trends
...
sig_df <- sig_df %>% filter(sig == 0)  # Retain only non-significant areas
...
ggplot() +
  ...
  scale_fill_distiller(
    palette = "RdBu", direction = -1,
    limits = c(-25, 25), name = "trends", na.value = "lightgrey", values = scales::rescale(c(-1, -0.1, 0, 0.1, 1))
  ) +
  ...
  # Overlay non-significant areas with hatching or stippling
  geom_point(data = sig_df, aes(x = x, y = y), shape = 4, size = 0.5, color = "black", alpha = 0.7) +  # '4' is an 'x' shape
...
```
```{r}
### plot distribution of trend values

# Convert raster data to a vector (excluding NA values)
mean_trend_values <- as.vector(r_df$layer)
summary(mean_trend_values)
mean_trend_values <- mean_trend_values[!is.na(mean_trend_values)]  # Remove NAs

# Create histogram with density overlay
ggplot(data.frame(value = mean_trend_values), aes(x = value)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50, fill = "blue", alpha = 0.5) +
  geom_density(color = "red", size = 1) +  # Add density curve
  labs(title = "Distribution of Mean ANNMAX PCWD Trend (1850-2009)",
       x = "Trend Value (mm/yr)", y = "Density") +
  theme_classic()
