...
saveRDS(regional_results_1420, file="~/cwd_global/data/regionalResults_1420.RData")
...
#save calculated list
saveRDS(regional_results_1850, file="~/cwd_global/data/regionalResults_1850.RData")
...
GIC_1420 <- regional_results_1420[["GIC"]]
GIC_1850 <- regional_results_1850[["GIC"]]

time_info_1420 <- seq(from = as.numeric(1420), to = as.numeric(1849))
time_info_1850 <- seq(from = as.numeric(1850), to = as.numeric(2009))

GIC_1420_mean <- data.frame(
  time = time_info_1420,  # Keep the time column
  PCWD = rowMeans(GIC_1420[, -1], na.rm = TRUE)  # Calculate row means, excluding the first column
)

GIC_1850_mean <- data.frame(
  time = time_info_1850,  # Keep the time column
  PCWD = rowMeans(GIC_1850[, -1], na.rm = TRUE)  # Calculate row means, excluding the first column
)
 
ggplot(data = GIC_1420_mean) +
  geom_line(mapping= aes(x=time, y=PCWD))+
  ylim(50, 70) +
  xlim(min(GIC_1420_mean$time), max(GIC_1420_mean$time)) +
  labs(
    x = "Year", 
    y = "PCWD(mm)",
    title = "GIC - EM PCWD values"
  ) +
  theme_classic()

ggplot(data = GIC_1850_mean) +
  geom_line(mapping= aes(x=time, y=PCWD))+
  ylim(50, 70) +
  xlim(min(GIC_1850_mean$time), max(GIC_1850_mean$time)) +
  labs(
    x = "Year", 
    y = "PCWD(mm)",
    title = "GIC - EM PCWD values"
...
skew_1420 <- skewness(as.vector(GIC_1420))
skew_1850 <- skewness(as.vector(GIC_1850))

# Calculate density
dx_1420 <- density(as.vector(GIC_1420))
dx_1850 <- density(as.vector(GIC_1850))
...
```
Calculate return level values for 30 year slices e.g. 1420-1450, 1600-1630, 1800-1830, 2000
- 30 year chunks
...
for (member_data in data_by_ensemble_1420) {
  # Extract the first 30 years
  data_subset <- member_data[,,380:(380+years_to_pool-1)] #edit 1 to whenever need to start e.g. for 1600 at 180:years to pool
...
# Create an empty matrix to store 5-year return levels
X5_1830 <- matrix(NA, nrow = length(lon), ncol = length(lat))
...
for (i in seq_along(lon)) {
  for (j in seq_along(lat)) {
   ...
   # if (all(is.na(data)) || length(na.omit(data)) < 10) next
    ...
    # Calculate 80-year return level
    X5_1830[i, j] <- calculate_return_level(data, return_period = 5)
  }
}

# Create an empty matrix to store 5-year return levels
X500_1830 <- matrix(NA, nrow = length(lon), ncol = length(lat))
...
for (i in seq_along(lon)) {
  for (j in seq_along(lat)) {
   ...
   # if (all(is.na(data)) || length(na.omit(data)) < 10) next
    ...
    # Calculate 80-year return level
    X500_1830[i, j] <- calculate_return_level(data, return_period = 500)
  }
}
...
# Convert first year's data into a raster object
r <- raster(t(X5_1630), xmn=min(lon), xmx=max(lon), ymn=min(lat), ymx=max(lat))
crs(r) <- "+proj=longlat +datum=WGS84"  # Set CRS for raster
r <- flip(r, direction='y')
...
ggplot() +
  ...
  scale_fill_manual(name = "5 yr return level", values = custom_colors, na.value = "lightgrey", 
                    labels = c(levels(cut(r_df$layer, breaks = custom_bins)),"Ocean")) +  # Label ocean
  labs(title = "PCWD ANNMAX X5 1600-1630", x = "Longitude", y = "Latitude") +
  theme_classic() +
  geom_sf(data = land, fill = NA, color = "black", lwd = 0.5) +
  coord_sf(xlim = range(lon), ylim = range(lat), expand = FALSE) +
  theme(legend.title = element_text(size = 12),  # Adjust legend title size
...
# Convert first year's data into a raster object
r <- raster(t(X5_1830), xmn=min(lon), xmx=max(lon), ymn=min(lat), ymx=max(lat))
crs(r) <- "+proj=longlat +datum=WGS84"  # Set CRS for raster
r <- flip(r, direction='y')
...
ggplot() +
  ...
  scale_fill_manual(name = "5 yr return level", values = custom_colors, na.value = "lightgrey", 
                    labels = c(levels(cut(r_df$layer, breaks = custom_bins)),"Ocean")) +  # Label ocean
  labs(title = "PCWD ANNMAX X5 1800-1830", x = "Longitude", y = "Latitude") +
  theme_classic() +
  geom_sf(data = land, fill = NA, color = "black", lwd = 0.5) +
  coord_sf(xlim = range(lon), ylim = range(lat), expand = FALSE) +
  theme(legend.title = element_text(size = 12),  # Adjust legend title size
...
# Convert first year's data into a raster object
r <- raster(t(X500_1630), xmn=min(lon), xmx=max(lon), ymn=min(lat), ymx=max(lat))
crs(r) <- "+proj=longlat +datum=WGS84"  # Set CRS for raster
r <- flip(r, direction='y')
...
ggplot() +
  ...
  scale_fill_manual(name = "5 yr return level", values = custom_colors, na.value = "lightgrey", 
                    labels = c(levels(cut(r_df$layer, breaks = custom_bins)),"Ocean")) +  # Label ocean
  labs(title = "PCWD ANNMAX X500 1600-1630", x = "Longitude", y = "Latitude") +
  theme_classic() +
  geom_sf(data = land, fill = NA, color = "black", lwd = 0.5) +
  coord_sf(xlim = range(lon), ylim = range(lat), expand = FALSE) +
  theme(legend.title = element_text(size = 12),  # Adjust legend title size
...
# Convert first year's data into a raster object
r <- raster(t(X500_1830), xmn=min(lon), xmx=max(lon), ymn=min(lat), ymx=max(lat))
crs(r) <- "+proj=longlat +datum=WGS84"  # Set CRS for raster
r <- flip(r, direction='y')
...
ggplot() +
  ...
  scale_fill_manual(name = "5 yr return level", values = custom_colors, na.value = "lightgrey", 
                    labels = c(levels(cut(r_df$layer, breaks = custom_bins)),"Ocean")) +  # Label ocean
  labs(title = "PCWD ANNMAX X500 1800-1830", x = "Longitude", y = "Latitude") +
  theme_classic() +
  geom_sf(data = land, fill = NA, color = "black", lwd = 0.5) +
  coord_sf(xlim = range(lon), ylim = range(lat), expand = FALSE) +
  theme(legend.title = element_text(size = 12),  # Adjust legend title size
...
```{r}
# Define the range for the gradient scale
value_range <- c(0, 4000)  # Adjust the range as needed
...
# Remove rows with NA values in `layer` to exclude ocean data from the color scale
r_df_filtered <- r_df[!is.na(r_df$layer), ]

# Define the ggplot2 object with the gradient scale and custom range
ggplot() +
  geom_raster(data = r_df_filtered, aes(x = x, y = y, fill = layer)) +
  scale_fill_gradientn(
    colours = terrain.colors(10),  # Use terrain.colors for gradient
    limits = value_range,          # Set the range for the gradient
    name = "5 yr return level",    # Label for the legend
    na.value = "lightgrey"         # Handle NA values (oceans) with a neutral color
  ) +
  labs(
    title = "PCWD ANNMAX X5 1420-1450",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_classic() +
