...
custom_bins <- c(0, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1200)  # Adjust bins for your data range
...
ggplot() +
  ...
  coord_sf(xlim = range(lon.wce), ylim = range(lat.wce), expand = FALSE) +
  ...
  guides(fill = guide_legend(title = "PCWD [mm]", reverse = TRUE))
...
WCE_sf <- st_as_sf(WCE)
# Ensure the CRS matches with the raster and other layers
WCE_sf <- st_transform(WCE_sf, crs = st_crs(r2.wce))
# Calculate centroids for text placement
WCE_centroids <- st_centroid(WCE_sf)
# Add a column for region names (if not already available)
WCE_centroids$region_name <- "WCE"  # Replace "WCE" with the actual region name if available
...
ggplot() +
  ...
  # Adjust legend presentation
  guides(fill = guide_legend(title = "PWCD [mm]", reverse = TRUE))
