...
ggplot(zscore_filtered, aes(x = time, y = mean_zscore, color = "PCWD Z-Scores")) +
  ...
  labs(
    ...
    title = paste("EM PCWD (annual) Z-Scores for", selected_continent)
  ) +
  # Secondary axis transformation ensuring zero alignment
  scale_y_continuous(
    sec.axis = sec_axis(
...
ggplot() +
  ...
  scale_fill_distiller(name = "EM z-scores (w.r.t. 1420-1450)", 
                       palette = "RdBu",  # Red-Blue diverging palette
                       limits = c(-3, 3),  # Adjust scale to focus on values between -1 and 1
  ...
  coord_sf(xlim = c(-120, -60)	, ylim = 	c(5, 35)	, expand = FALSE) +  # Adjusted for Europe
