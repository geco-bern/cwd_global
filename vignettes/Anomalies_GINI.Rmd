...
ggplot(anom_filtered, aes(x = time, y = anomalies, color = "PCWD anomalies")) +
  ...
  labs(
    x = "Year",
    y = "PCWD Anomalies (w.r.t. 1420-1450)",
...
year_index <- 1849 - 1420 + 1  #-1420 or 1850 depending on dataset
...
ggplot() +
  ...
  scale_fill_distiller(name = "PCWD [mm]", 
                       ...
                       values = scales::rescale(c(-1200, -50, 50, 1200)),  # Rescales to emphasize 0-2 range
                       limits = c(-3000, 3000),  # Adjust scale to focus on values between -1 and 1
                       na.value = "lightgrey") +  # Color for missing values
  labs(title = "PCWD Anomalies for 1849 w.r.t 1420-1450", x = "Longitude", y = "Latitude") +
  theme_classic() +
...
# Compute mean anomalies and region-specific ±2σ bounds
mean_anomalies_filtered <- anoms_filtered %>%
  group_by(time, region) %>%
  summarise(
    mean_anomaly = mean(anomalies, na.rm = TRUE),
    pooled_sd = pooled_sd_by_region[[unique(region)]],  # Use the correct SD for each region
    lower_bound = mean_anomaly - 2 * pooled_sd,
    upper_bound = mean_anomaly + 2 * pooled_sd
  ) %>%
  ungroup()
...
ggplot() +
  ...
  scale_color_manual(
    ...
    labels = c("± 2σ", "Ensemble Members", "PCWD (Mean)")
  ) +
...
mean_anomalies_filtered <- anoms_filtered %>%
  ...
  summarise(
    mean_anomaly = mean(anomalies, na.rm = TRUE),
    pooled_sd = pooled_sd_by_region_1420[[unique(region)]],  # Use the correct SD for each region
    lower_bound = mean_anomaly - 2 * pooled_sd,
    upper_bound = mean_anomaly + 2 * pooled_sd
  ) %>%
  ungroup()
