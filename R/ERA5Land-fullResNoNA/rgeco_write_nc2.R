# THIS HAS BEEN TAKEN FROM rgeco PACKAGE.
# AND IT WAS ADAPTED FOR WRITING OUT VERY LARGE ARRAYS IN SLICE FOR EACH TIME
# THIS AVOIDS CASES WHEN THE GRIDDED DATA CONTAINS MORE THAN 2^31 values, as 
# is the case for a year of daily ERA5LAND (0.1 deg): 3601*1801*365 = 2'367'171'365
# FB: 2026-03-06
...
rgeco_write_nc2 <- function(
  ...
  ## If 'obj' is a rbeni-nc element, then no additional info about dimensions must be provided
  if (any(class(obj) == "list")){

    if (is.element("vars", ls(obj)) && is.element("lat", ls(obj)) && is.element("lon", ls(obj))){

      if (!identical(NA, varnams)){
        var <- obj$vars[ varnams ] # NOTE: this
      } else {
  ...
  if (length(var[[1]]) < 2^31 - 1){
    # THIS BREAKS DOWN WITH 365x3601x1801 = 2'367'171'365  values, since it is larger than 2^31 - 1 = 2'147'483'647
    purrr::map(as.list(seq(length(varnams))),
               ~ncdf4::ncvar_put( nc, varid[[.]], var[[.]], start = NA, count = NA, verbose = verbose ))
  } else {
    warning("Array is too large to be writting in one go to NetCDF. Attempt to write by time slice.")
    if(!make_tdim){stop("Array is too large to be written in one go to NetCDF. But can't write by time slice since no time dimension present.")}
    for (var_it in seq_along(varnams)){
      arr <- array( # NOTE: bring it back to a 3D array for sliced writing.
                    #       This appears to work even with overlarge arrays.
        var[[var_it]],
        dim = c(lonid$len, latid$len, timeid$len))
      #check that this makes sense (time series): plot(arr[78, 1362, ])
      #check that this does not make sense: plot(arr[78, , 180])
      #check that this does not make sense: plot(arr[, 1362, 180])

      for (t_it in seq_len(timeid$len)) {
        slice <- arr[ , , t_it, drop = FALSE]
        # stopifnot(dim(slice) == c(3601L, 1801L, 1L))
        ncdf4::ncvar_put(
          nc,
          varid[[var_it]],
          slice,
          start=c(1,1,t_it),
          count=c(lonid$len, latid$len, 1),
          verbose = FALSE)
      }
    }
  }
