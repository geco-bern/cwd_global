...
ModESim_compute_pcwd_byLON <- function(
  ...
  indir_patm      <- file.path(indir, "1850_01_m014_patm")
  indir_tas       <- file.path(indir, "1850_01_m014_tsurf")
  indir_prec      <- file.path(indir, "1850_01_m014_precip")
  indir_netrad    <- file.path(indir, "1850_01_m014_netrad")
  ...
  filnam <- file.path(indir_patm, paste0("set1850_1_m014_patm_",
                                            LON_string,".rds"))
  df_patm <- readr::read_rds(filnam)


  # read precipitation file tidy
  filnam <- file.path(indir_prec, paste0("set1850_1_m014_precip_",
                                         LON_string,".rds"))
  df_prec <- readr::read_rds(filnam)

  # read temperature file tidy
  filnam <- file.path(indir_tas, paste0("set1850_1_m014_tsurf_",
                                         LON_string,".rds"))
  df_tas <- readr::read_rds(filnam)

  # read net radiation file tidy
  filnam <- file.path(indir_netrad, paste0("set1850_1_m014_netrad_",
                                        LON_string,".rds"))
